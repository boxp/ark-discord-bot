...
(deftest test-format-status-message
  (testing "format-status-message creates readable output"
    (let [msg (status/format-status-message
               {:status :running
                :k8s {:ready 1}
...
(clojure.test/run-tests 'ark-discord-bot.core.status-test)

...
(deftest test-recovery-debounce
  ...
  (testing "recovery notifies when failure notification was sent even within cooldown"
    (let [base-time 1000000
          state (-> (monitor/create-state 3 default-cooldown-ms)
                    (monitor/update-state :running base-time)
      ...
      (is (monitor/should-notify-with-debounce?
           state :running 0 (+ base-time (* 2 60 1000)))))))
